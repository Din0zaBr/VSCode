...
# ── 7. Write wrapper .bat (handles PYTHONPATH — most reliable on PS 5.1) ──────
Write-Step "Writing launcher script..."

$launcherPath = Join-Path $DataDir "run-agent.bat"
@"
@echo off
REM LogVault Agent launcher — auto-generated by install-windows.ps1
REM Sets PYTHONPATH so Python finds the 'agent' package, then starts the agent.

set PYTHONPATH=$SourceRoot
cd /d "$SourceRoot"
"$PythonExe" -m agent.src.main "$configPath"
"@ | Set-Content -Encoding ASCII -Path $launcherPath

Write-OK "Launcher written to $launcherPath"

# ── 8. Remove stale task if present ───────────────────────────────────────────
Write-Step "Registering Scheduled Task '$TaskName'..."

schtasks /delete /tn "$TaskName" /f 2>$null | Out-Null

# ── 9. Register via schtasks.exe (works on PS 5.1 without XML tricks) ─────────
# /sc ONSTART  = at system startup
# /ru SYSTEM   = run as SYSTEM (full Security log access)
...
Write-OK "Task '$TaskName' registered (runs as SYSTEM at every boot)"

# ── 10. Start task immediately ────────────────────────────────────────────────
Write-Step "Starting task now..."
schtasks /run /tn "$TaskName"
Start-Sleep -Seconds 3
$statusLine = (schtasks /query /tn "$TaskName" /fo LIST 2>&1) |
    Select-String "Status|Состояние"
Write-OK "Task status: $statusLine"
...
Write-Host " Runs as     : SYSTEM (full Security log access)"
Write-Host " Auto-start  : Yes (at every boot)"
Write-Host ""
...
Write-Host "   Invoke-WebRequest -Uri '$ServerUrl/api/search?service=windows-security&size=5'"
Write-Host ""
