...
pub fn translate(
    ...
    let select = if !group_cols.is_empty() {
        ...
        "id, event_id, timestamp, host, agent_id, source, level, message, service, meta".to_string()
    };
    ...
    if sort_parts.is_empty() {
        ...
        let final_sort: Vec<String> = if !group_cols.is_empty() {
            // When GROUP BY active, timestamp not in GROUP BY — wrap in MAX()
            sort_parts.iter().map(|p| {
                if p == "timestamp DESC" { "MAX(timestamp) DESC".to_string() }
                else if p == "timestamp ASC" { "MAX(timestamp) ASC".to_string() }
                else { p.clone() }
            }).collect()
        } else {
            sort_parts.clone()
        };
        sql.push_str(&format!(" ORDER BY {}", final_sort.join(", ")));
    }
