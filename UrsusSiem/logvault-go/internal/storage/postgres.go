...
// PoolForJobs exposes the underlying pgxpool so background jobs in
// `internal/jobs/` can run multi-statement transactions without the
// LogRepo interface getting bloated with pool-level methods.
//
// Returns nil when the DB is closed — callers should handle that.
func (db *DB) PoolForJobs() *pgxpool.Pool { return db.pool }
...
// HostSummary represents a host aggregation derived from logs metadata,
// used by the /api/hosts endpoint (lightweight, read-only).
// Persistent asset records live in the `assets` table — see assets.go.
...
// QueryHostsFromLogs returns distinct hosts and their metadata from logs.
func (db *DB) QueryHostsFromLogs(ctx context.Context) ([]HostSummary, error) {
	rows, err := db.pool.Query(ctx, `
		SELECT host,
